  ...
  (req, res) => {
    try {
      ...
      if (directories.length === 1) {
        const folderName = directories[0].substring(
          directories[0].lastIndexOf("/") + 1
        );
        const redirectUrl = `http://jantschulev.ddns.net/projects/${endpoint}/${folderName}/`;
        fs.writeFileSync(
          path.join(folderPath, "index.html"),
          `<html><body><a href="${redirectUrl}">redirect</a><script>window.location.href="${redirectUrl}"</script></body></html>`
        );
